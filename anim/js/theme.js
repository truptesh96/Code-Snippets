...
jQuery(function($) {
	...
	var sliderScrl = $('.fadeSlides').slick({
	  dots: true, vertical: true, verticalSwiping: true, arrows: false, dots: false,
	  infinite: true, vertical: true, speed: 500, loop: false, infinite: false, cssEase: 'linear'
	  // fade: true,
	  // cssEase: 'linear'
	}).on("afterChange", function (){
	    var dataId = $('.slick-current').attr("data-slick-index");
   		$(this).parents('.thumbsliderWrap').find('.thumbRows .sslide').eq(dataId).addClass('isActive').siblings().removeClass('isActive');
	});


	$('.thumbRows .sslide').click(function(){
		$(this).parents('.thumbsliderWrap').find('.fadeSlides').slick('slickGoTo', $(this).index());
	});
